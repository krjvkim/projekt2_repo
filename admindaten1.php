...
if (isset($_SESSION['session_user'])) {
    ...
    $liste1 = array('Benutzername: ', 'benutzername', '1');
    $liste2 = array('Passwort: ', 'passwort', '2');
    ...
    if (isset($_POST['submit_eingabeformular']))	// !Submit-Schaltfläche des Eingabeformulars wurde betätigt
    {
        ...
        $query="UPDATE admindaten SET /*id='$_POST[id]',*/ benutzername='$_POST[benutzername]', passwort='$_POST[passwort]', vorname='$_POST[vorname]', nachname='$_POST[nachname]', datumeintrag = '$datum', letztebearbeitungvon = '$_SESSION[session_user]' WHERE id='$_GET[id]'";
    ...
    if (isset($_POST['submit_neuezeile']))	// Submit-Schaltfläche des Eingabeformulars wurde betätigt
    {
        // SQL-Kommando: Ändern von Einträgen
        $id = intval($_GET['id']);
        $query="INSERT INTO admindaten (id, benutzername, passwort, vorname, nachname, letztebearbeitungvon) VALUES ($id,'$_POST[benutzername]', '$_POST[passwort]', '$_POST[vorname]', '$_POST[nachname]', '$datum', '$_SESSION[session_user]')";
        // SQL-Kommando ausführen
        pg_query($con, $query);
    }
