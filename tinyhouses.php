...
<head>
    ...
    <title>Adminportal - Tinyhouses</title>
...
if (isset($_SESSION['session_user'])) { 
    ...
    $tabelle = 'tinyhouses';
    $datei = 'tinyhouses.php';

   /* $liste1 = array('Tinyhouse-ID: ', 'id', '1');*/
    $liste1 = array('Bezeichnung: ', 'bezeichnung', '1');
    $liste2 = array('Baudatum: ', 'baudatum', '2');
    $liste3 = array('Eigentümer: ', 'eigentuemer', '3');
    $liste4 = array('Siedlung-ID: ', 'siedlungid', '4');
    $liste = array($liste1, $liste2, $liste3, $liste4);
    ...
    if (isset($_POST['submit_eingabeformular']))	// Submit-Schaltfläche des Eingabeformulars wurde betätigt
    {
        ...
        $query="UPDATE tinyhouses SET bezeichnung='$_POST[bezeichnung]', baudatum='$_POST[baudatum]', eigentuemer='$_POST[eigentuemer]', siedlungid='$_POST[siedlungid]', datumeintrag = '$datum', letztebearbeitungvon = '$_SESSION[session_user]' WHERE id='$_GET[id]'";
    ...
    if (isset($_POST['submit_neuezeile']))	// Submit-Schaltfläche des Eingabeformulars wurde betätigt
    {
        // SQL-Kommando: Ändern von Einträgen
        $id = intval($_GET['id']);
        $query="INSERT INTO tinyhouses (id, bezeichnung, baudatum, eigentuemer, siedlungid, datumeintrag, letztebearbeitungvon) VALUES ($id,'$_POST[bezeichnung]','$_POST[baudatum]','$_POST[eigentuemer]','$_POST[siedlungid]', '$datum', '$_SESSION[session_user]')";
        // SQL-Kommando ausführen
        pg_query($con, $query);
    }
    if (isset($_POST['submit_eintragloeschen']))	// Submit-Schaltfläche des Eingabeformulars wurde betätigt
    {
        ...
        $query="DELETE FROM tinyhouses WHERE id='$_GET[id]'";
        // SQL-Kommando ausführen
        pg_query($con, $query);
    }

    $query = "SELECT * FROM tinyhouses ORDER BY id";
    ...
    echo "<thead>"; echo "<tr>"; echo "<th>Tinyhouse-ID</th>"; echo "<th>Bezeichnung</th>"; echo "<th>Baudatum</th>"; echo "<th>Eigentümer</th>"; echo "<th>Siedlung-ID</th>"; echo "<th>Datum letzte Bearbeitung</th>"; echo "<th>Letzte Bearbeitung durchgeführt von</th>"; echo"</tr>"; echo"</thead>";
